...
# ✅ NEW FUNCTION (OUTSIDE, NOT INSIDE)
...
def verify_document():
    try:
        ...
        issuer_username = request.form.get('issuerUsername', 'unknown') # Added to track who verified it
        ...
        if not document_id or not ObjectId.is_valid(document_id):
            return jsonify({"message": "Valid Document ID required"}), 400
        ...
        # ✅ Extract details
        name, extracted_id = extract_details(ocr_text)
        ...
        if score >= 80:
            result_text = "GENUINE"
            db_status = "valid"      # Status to save in DB
        elif score >= 60:
            result_text = "SUSPICIOUS"
            db_status = "suspicious" # Status to save in DB
        else:
            result_text = "FAKE"
            db_status = "invalid"    # Status to save in DB

        details = f"File: {original_name}, Type: {mime_type}, Size: {size_bytes}, Similar docs: {similar_docs}"
        ...
        update_data = {
            ...
            "aiAnalysis": {
                ...
                "similar_docs_found": similar_docs
            },
            "verifiedAt": datetime.now(),
            "verifiedBy": issuer_username
        }
        
        # Optional: If the issuer typed a name, update the document's fullName field too
        if holder_name:
            update_data["fullName"] = holder_name

        db_result = documents_collection.update_one(
            {"_id": ObjectId(document_id)},
            {"$set": update_data}
        )
        
        if db_result.matched_count == 0:
            print(f"⚠️ Warning: Document {document_id} was not updated in DB.")
        else:
            print(f"✅ Document {document_id} updated to status: {db_status}")
        # =====================================================================
        ...
        return jsonify({
            "score": score,
            "result": result_text,
            "verified": score >= 80,  # This will be true for valid, false for suspicious/invalid
            "status": db_status,         # ✅ Send the new status back to frontend
            "documentId": document_id,   # ✅ Send ID back so frontend can trigger QR generation
            "details": details,
            "ocr_text": ocr_text,
            "extracted_name": name,
            "extracted_id": extracted_id,
          "db_check": db_result.matched_count > 0 # ✅ Now accurately reflects DB update
        }), 200

    except Exception as e:
        print("Verify error:", e)
        import traceback
        traceback.print_exc() # Added full traceback for easier debugging
